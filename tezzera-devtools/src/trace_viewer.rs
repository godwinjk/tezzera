...
use tezzera_trace::event::{TezzeraTrace, TraceValue};

/// Formats TezzeraTrace events as a human-readable ASCII log.
...
impl TraceViewer {
    ...
    pub fn drain(&mut self, rx: &Receiver<TezzeraTrace>) {
    ...
    pub fn render(&self) -> String {
        ...
        let mut out = String::from("┌─ TEZZERA TRACE ─────────────────────────────────\n");
...
fn format_event(ev: &TezzeraTrace) -> String {
    match ev {
        TezzeraTrace::ComponentMount { id, name, location } => {
            format!("ComponentMount  id={} name={} @ {}:{}", id.0, name, location.file, location.line)
        }
        TezzeraTrace::ComponentUnmount { id, name } => {
            format!("ComponentUnmount  id={} name={}", id.0, name)
        }
        TezzeraTrace::ComponentRebuild { id, cause, duration } => {
        ...
        TezzeraTrace::AtomWrite { atom, old, new, by, location } => {
            format!(
        ...
        TezzeraTrace::RequestStart { id, url, method, component } => {
...
mod tests {
    ...
    use tezzera_trace::event::{ComponentId, Location, TezzeraTrace};
    ...
    fn trace_viewer_drain_empty_channel() {
        ...
        let (_tx, rx) = mpsc::channel::<TezzeraTrace>();
        viewer.drain(&rx);
        assert_eq!(viewer.event_count(), 0);
    }
    ...
    fn trace_viewer_last_returns_recent() {
        let mut viewer = TraceViewer::new();
        let (tx, rx) = mpsc::channel::<TezzeraTrace>();
        tx.send(TezzeraTrace::ComponentMount {
            id: ComponentId(1),
            name: "Alpha",
            location: Location { file: "a.rs", line: 1 },
        })
        .unwrap();
        tx.send(TezzeraTrace::ComponentUnmount {
            id: ComponentId(2),
            name: "Beta",
        })
        .unwrap();
    ...
    fn trace_viewer_respects_max_events() {
        ...
        for i in 0..5u64 {
            tx.send(TezzeraTrace::ComponentUnmount {
                id: tezzera_trace::event::ComponentId(i),
                name: "X",
            })
            .unwrap();
        }
